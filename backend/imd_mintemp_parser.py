"""
IMD Daily Minimum Temperature Grid Parser
File format: Mintemp_MinT_YYYY.GRD
Resolution:  1.0° × 1.0°
...
NROWS      = 31
CELLSIZE   = 1.0
XLLCORNER  = 67.0   # western edge (cell centre = 67.5)
YLLCORNER  = 7.0    # southern edge (cell centre = 7.5)
NODATA     = 99.9    # IMD MinTemp missing value is +99.9 (NOT -99.9)
CELLS_DAY  = NCOLS * NROWS   # 961


def _year_from_filename(filepath: str) -> int:
    """Extract year from Mintemp_MinT_YYYY.GRD  (case-insensitive)."""
...
class IMDMinTempParser:
    """
    Parse IMD daily minimum temperature .GRD files.
    Usage:
        parser = IMDMinTempParser('Mintemp_MinT_2017.GRD')
        parser.parse()
        data = parser.get_day_data(0)   # day index 0 = Jan 1
    """
    ...
    def __init__(self, filepath: str, start_date: str = None):
        ...
        # Grid metadata (mirrors rainfall parser interface)
        self.ncols      = NCOLS
        self.nrows      = NROWS
    ...
    def parse(self):
        ...
        total   = len(raw) // 4
        n_days  = total // CELLS_DAY
        self.n_days = n_days   # update from actual file

        flat = np.frombuffer(raw[:n_days * CELLS_DAY * 4],
                             dtype='<f4').astype(float)
        flat[flat >= NODATA - 0.05] = np.nan    # mask nodata (99.9 = ocean/missing)
    ...
    def get_statistics(self, day_idx: int = None) -> dict:
        """
        Stats for one day (day_idx given) or entire dataset (day_idx=None).
        Returns dict with min/max/mean/median/std/count.
        """
        if self._data is None:
            raise RuntimeError("Call parse() first.")
    ...
    def get_point_value(self, longitude: float, latitude: float,
        ...
        """
        Nearest-neighbour lookup for a (lon, lat) point on a given day.
        Returns °C value or None if out of bounds / nodata.
        """
        if self._data is None:
            raise RuntimeError("Call parse() first.")
    ...
    def get_timeseries(self, longitude: float, latitude: float,
                       from_day: int = 0, to_day: int = None) -> list[dict]:
        """
        Time-series of min temperature for a point over a day range.
        Returns list of {date, mintemp_c}.
        """
        if self._data is None:
            raise RuntimeError("Call parse() first.")
        ...
        for idx in range(from_day, to_day + 1):
            ...
            result.append(dict(date=d.isoformat(), mintemp_c=val))

        return result

    def iter_day_rows(self, day_idx: int):
        """
        Generator: yields (lat, lon, mintemp_c) for every valid cell
        in a day. Used for CSV export.
        """
        if self._data is None:
            raise RuntimeError("Call parse() first.")
