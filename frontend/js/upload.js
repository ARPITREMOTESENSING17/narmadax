...
(function injectCalendar() {
  ...
  style.textContent = `
    ...
    .cal-pill.to-filled   .pill-val { color:#38bdf8; }
...
async function uploadFile() {
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (data.file_type === 'imd_daily') {
        statusDiv.innerHTML +=
          `<p style="color:#38bdf8;">📅 ${currentNDays} days · ` +
...
function displayResults(data) {
  ...
  const stats = data.file_type === 'imd_daily'
    ? data.statistics_first_day
    : data.statistics;
  ...
  try {
    ...
    if (currentFileType === 'imd_daily') {
      ...
      if (result.isAll) {
        ...
        url      = `/api/export/${currentFileId}/csv_all`;
        filename = `${currentFileId}_all_days.csv`;
      } else {
        url =
          `/api/export/${currentFileId}/csv_range` +
      ...
      url      = `/api/export/${currentFileId}/${format}`;
      filename = `${currentFileId}.${ext[format]}`;
    }

    // ── Direct download — no fetch, no blob, no memory limit ──────────
    // Browser streams directly to disk, works for files of any size
    ...
    if (currentFileType === 'imd_daily') {
      ...
      if (response.ok) {
        ...
        document.getElementById('pointResult').innerHTML = `
          ...
          <p>Total rainfall : <strong>${total.toFixed(2)} mm</strong></p>
          <p>Average daily  : ${avg.toFixed(2)} mm</p>
          <p>Maximum daily  : ${max.toFixed(2)} mm</p>
          <p>Wet days (>0)  : ${values.filter(v => v > 0).length} / ${nDays}</p>
        `;
      } else {
