...
(function injectCalendar() {
  ...
  style.textContent = `
    ...
    .cal-pill.to-filled   .pill-val { color:#e05c1a; }
...
async function maxtempUploadFile() {
  ...
  try {
    const response = await fetch('/api/maxtemp/upload', {
      method: 'POST',
    ...
    if (response.ok) {
      ...
      maxtempDisplayResults(data);
      statusDiv.innerHTML = '<p style="color:green;">File processed successfully!</p>';

      if (data.file_type === 'imd_maxtemp') {
        statusDiv.innerHTML +=
          `<p style="color:#e05c1a;">📅 ${currentNDays} days · ` +
...
function maxtempDisplayResults(data) {
...
async function maxtempExportData(format) {
  if (!currentFileId) { alert('Please upload a file first'); return; }

  try {
    let url, filename;

    if (currentFileType === 'imd_maxtemp') {
      if (format !== 'csv') {
      ...
      if (result.isAll) {
        ...
        url      = `/api/maxtemp/export/${currentFileId}/csv_all`;
        filename = `${currentFileId}_all_days.csv`;
      } else {
        url =
          `/api/maxtemp/export/${currentFileId}/csv_range` +
      ...
      url      = `/api/maxtemp/export/${currentFileId}/${format}`;
...
async function maxtempQueryPoint() {
  if (!currentFileId) { alert('Please upload a file first'); return; }
  ...
  try {
    if (currentFileType === 'imd_maxtemp') {
      ...
      const response = await fetch(
        `/api/maxtemp/timeseries/${currentFileId}`, {
          method:  'POST',
      ...
      if (response.ok) {
        ...
        document.getElementById('pointResult').innerHTML = `
          ...
          <p>Avg max temp : <strong>${total.toFixed(2)} °C</strong></p>
          <p>Min daily    : ${avg.toFixed(2)} °C</p>
          <p>Max daily    : ${max.toFixed(2)} °C</p>
          <p>Days > 40°C   : ${values.filter(v => v > 40).length} / ${nDays}</p>
        `;
      } else {
